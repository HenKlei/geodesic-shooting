...
if __name__ == "__main__":
    ...
    input_ = loadimg('../example_images/circle.png')
    target = loadimg('../example_images/square.png')
    ...
    plt_inverse = plot_warpgrid(Phi0, title="Inverse warp grid (C2S)", interval=2)
    plt_inverse.savefig(FILEPATH_RESULTS + 'circle_to_square_warp_inverse.png')
    ...
    saveimg(FILEPATH_RESULTS + 'circle_to_square_half_speed.png', lddmm.push_forward(input_, Phi_half))

    # plot the (initial) vector field
    plt = plot_vector_field(v0, title="Initial vector field (C2S)", interval=2)
    plt.show()
