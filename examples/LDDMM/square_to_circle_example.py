...
if __name__ == "__main__":
    ...
    problem = pyLDDMM.ImageRegistrationProblem(i0, alpha=1, gamma=1)

    # perform the registration
    lddmm = pyLDDMM.LDDMM()
    im, v, energies, Phi0, Phi1, J0, J1, length = lddmm.register(i1, problem, sigma=0.1, epsilon=0.0001, K=50, return_all=True)

    # save i0 aligned to i1
    saveimg('../example_images/out_s2c.png', im)

    # save animation of the transformation
    save_animation('../example_images/out_s2c.gif', J0)

    # plot the transformation
    plt = plot_warpgrid(Phi1[0], interval=1)
    plt.savefig('../example_images/out_s2c_warp.png')
