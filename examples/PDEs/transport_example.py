...
_, _, u_sorted = zip(*sorted(zip(dof_x, dof_y, u_orig[1].to_numpy().squeeze())))
_, _, u_sorted_end = zip(*sorted(zip(dof_x, dof_y, u_orig[-1].to_numpy().squeeze())))
  
u_transformed = np.reshape(u_sorted, (N_x+1, -1))
u_transformed_end = np.reshape(u_sorted_end, (N_x+1, -1))

fig, axes = plt.subplots()
cf = axes.contourf(x_transformed, y_transformed, u_transformed, 100, cmap='jet')
axes.axis('equal')
plt.tight_layout()
#plt.show()

fig, axes = plt.subplots()
cf = axes.contourf(x_transformed, y_transformed, u_transformed_end, 100, cmap='jet')
axes.axis('equal')
plt.tight_layout()
#plt.show()
...
problem = pyLDDMM.PDEProblemInstationary(fom, u_transformed, implicit_Euler=False, alpha=1, gamma=1)
