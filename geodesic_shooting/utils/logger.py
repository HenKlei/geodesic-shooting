...
class ColoredFormatter(logging.Formatter):
    ...
    `GEODESIC_SHOOTING_COLORS_DISABLE` environment variable to `1`.
    """

    def __init__(self):
        disable_colors = int(os.environ.get('GEODESIC_SHOOTING_COLORS_DISABLE', 0)) == 1
        if disable_colors:
...
def getLogger(module, level=None, filename=''):
    ...
    filename
        ...
        written to.
    """
    module = 'geodesic_shooting' if module == '__main__' else module
...
def set_log_levels(levels=None):
    ...
    """
    levels = levels or {'geodesic_shooting': 'INFO'}
